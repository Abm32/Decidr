...
export default function Page() {
  ...
  // conversation: start session on entry
  useEffect(() => {
  ...
  const retryAudio = useCallback(() => {
    store.setApiState('generateAudio', { status: 'idle' });
  }, [store]);

  const retryComparison = useCallback(() => {
    store.setApiState('getComparison', { status: 'idle' });
  }, [store]);

  const retryConversation = useCallback(() => {
    store.setApiState('startConversation', { status: 'idle' });
  }, [store]);
  ...
  const stepContent: Record<Step, React.ReactNode> = {
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {scenarioSet?.map((s, i) => (
          <ScenarioCard
            key={s.scenario_id}
            scenario={s}
            isSelected={s.scenario_id === selectedScenarioId}
            onSelect={() => handleSelectScenario(s.scenario_id)}
    ...
    'audio-experience': (
      <AudioPlayer
        ...
        isLoading={apiAudio.status === 'loading'}
        error={errFrom(apiAudio)}
        onRetry={retryAudio}
    ...
    'conversation': (
      <ConversationInterface
        ...
        isConnecting={apiStartConvo.status === 'loading'}
        error={errFrom(apiStartConvo) ?? errFrom(apiConversation)}
    ...
    'comparison': (
      <ComparisonView
        ...
        isLoading={apiComparison.status === 'loading'}
        error={errFrom(apiComparison)}
    ...
    <main className="mx-auto max-w-5xl px-4 py-6">
      ...
      <ErrorBoundary fallback={<p className="p-4 text-red-400">Something went wrong. Please refresh.</p>}>
        <AnimatePresence mode="wait">
          <motion.div key={currentStep} {...fade} className="mt-6">
